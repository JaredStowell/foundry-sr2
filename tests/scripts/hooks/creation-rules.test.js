...
describe("registerCreationRuleHooks", () => {
    ...
    it("prevents unsetting completion/creation flags once locked", () => {
        ...
        expect(foundry.utils.getProperty(changes, "flags.shadowrun2e.-=creationCompleted")).toBeUndefined();
    ...
    it("blocks contact creation when nuyen budget would go negative", () => {
        ...
        expect(result).toBe(false);
        expect(ui.notifications.error).toHaveBeenCalledWith("Not enough creation Nuyen remaining for that contact.");
    });
    ...
    it("clamps contact level and blocks expensive contact updates", () => {
        ...
        expect(result).toBe(false);
        expect(ui.notifications.error).toHaveBeenCalledWith("Not enough creation Nuyen remaining for that contact change.");
    });
    ...
    it("blocks contact updates for cap violations returned by summary helper", () => {
        ...
        const actor = createContactActor();
        const result = contactUpdateHook(
            actor,
            { system: { details: { contactLevel: 2 } } },
            {},
            "U1"
        );
    ...
    it("rejects creating Sorcery skills when magic is zero", () => {
        ...
        expect(ui.notifications.error).toHaveBeenCalledWith("Sorcery and Conjuring require a Magic rating.");
    });
    ...
    it("clamps spell force updates to 1..6 during creation mode", () => {
        ...
        expect(ui.notifications.warn).toHaveBeenCalledWith("In creation mode, spell Force must be between 1 and 6.");
    });
    ...
    it("rejects spell creation with out-of-range force in creation mode", () => {
        ...
        expect(results.some(result => result === false)).toBe(true);
        expect(ui.notifications.error).toHaveBeenCalledWith("In creation mode, spell Force must be between 1 and 6.");
    });
    ...
    it("blocks expensive non-skill item creation and updates in creation mode", () => {
        ...
        expect(ui.notifications.error).toHaveBeenCalledWith("Not enough creation Nuyen remaining for that item.");
        ...
        expect(ui.notifications.error).toHaveBeenCalledWith("Not enough creation Nuyen remaining for that change.");
    });
    ...
    it("handles force point update no-op branches cleanly", () => {
        ...
        const gearDecrease = runAll(preUpdateItemHooks, [gear, { system: { quantity: 1, price: 100000 } }, {}, "U1"]);
    ...
    it("ignores hook processing when updates come from another user or skip budget flag", () => {
        ...
        expect(foundry.utils.getProperty(lockChanges, "flags.shadowrun2e.creationCompleted")).toBe(false);
