...
describe("sr2SkillInferAllocatedRating", () => {
  ...
  it("supports legacy rating and conc/spec fallbacks", () => {
    ...
    expect(sr2SkillInferAllocatedRating({ specialization: "Heavy", specializationRating: 6 })).toBe(
      4,
    );
    expect(sr2SkillInferAllocatedRating({ specialization: "Heavy", concentrationRating: 3 })).toBe(
      3,
    );
...
describe("sr2ParseFocusName", () => {
  it("parses focus naming conventions and rejects invalid names", () => {
    expect(sr2ParseFocusName("Spell Lock")).toEqual({
      kind: "spell lock",
      rating: 0,
      name: "Spell Lock",
    });
...
describe("sr2 skill/force/spell lock helpers", () => {
  ...
  it("infers spell lock augmentation modifiers", () => {
    expect(
      sr2InferSpellLockAugmentationModifiersFromSpellName("Increase Reflexes +2 dice"),
    ).toEqual({ INI: 2 });
    ...
    const modifiers = sr2ComputeSpellLockAugmentationModifiers([
      ...
      {
        type: "spell",
        name: "Increase Reflexes +3 dice",
        system: { spellLock: { enabled: true } },
      },
      {
        type: "spell",
        name: "Increase Reflexes +2 dice",
        system: { spellLock: { enabled: false } },
      },
    ]);
