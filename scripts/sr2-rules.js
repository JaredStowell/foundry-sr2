...
export function sr2NormalizeContactLevel(value) {
  ...
  return Math.max(
    SR2_CONTACT_LEVEL_RULES.minLevel,
    Math.min(SR2_CONTACT_LEVEL_RULES.maxLevel, num),
  );
}
...
export function sr2ComputeContactLevelSummary(contacts, charisma, options = {}) {
  ...
  const exemptedLevel2 =
    exemptedLevel3 + Math.min(remainingSlots, Math.max(0, totalLevel2 - exemptedLevel3));
...
export function sr2ComputeCreationExtrasCost(extras, options = {}) {
  ...
  const contactsCost =
    contactLevelsSummary && typeof contactLevelsSummary?.costs?.contactsTotalCost === "number"
...
export function sr2ComputeCreationNuyenBudgetBreakdown(system, items, options = {}) {
  ...
  const lifestyles =
    Array.isArray(rawLifestyles) && rawLifestyles.length
      ? rawLifestyles.map((l) =>
          sr2ComputeCreationLifestyleCost(l?.type || "street", l?.months ?? 1),
        )
      : [
          sr2ComputeCreationLifestyleCost(
            system?.resources?.lifestyle || "street",
            system?.creation?.lifestyleMonths ?? 1,
          ),
        ];
...
export function sr2InferSpellLockAugmentationModifiersFromSpellName(spellName) {
  ...
  const increaseReflexesMatch = name.match(
    /^Increase Reflexes\s*\+(\d+)\s*(?:initiative\s+)?(?:die|dice)$/i,
  );
