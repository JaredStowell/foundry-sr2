...
export class SR2GearPurchaseApp extends Application {
    ...
    async getData() {
        ...
        return {
            ...
            rows: this.rows.map(r => ({
                ...r,
                unitCostDisplay: r.unitCostValue === null ? (r.unitCostDisplay || "?") : this._formatNuyen(r.unitCostValue),
                subtotalDisplay: r.unitCostValue === null ? "?" : this._formatNuyen(r.unitCostValue * r.quantity),
    ...
    _resolveCatalogItem({ itemType, name, expectedPrice, gearCatalogIndex }) {
        ...
        if (!index) return { name, cost: { value: null, display: "?" }, canBuy: false, note: "Unknown item type" };
        ...
        if (!matches.length) {
            return { name, cost: { value: null, display: "?" }, canBuy: false, note: "Not found in catalog" };
        }
        ...
        if (!scored.length) {
            return { name, cost: { value: null, display: matches[0]?.cost ?? "?" }, canBuy: false, note: "Cost not numeric" };
        }
    ...
    async _onPurchase(event) {
        ...
        const buyerNuyen = this._getActorNuyen(buyer);
        const selectedRows = this.rows.filter(r => r.selected && r.canBuy && r.unitCostValue !== null && r.catalogItem);
        ...
        ui.notifications.info(`Purchased ${itemsToCreate.length} item(s) for ¥${this._formatNuyen(totalCost)}.`);
        await this.close();
    }
    ...
    _createSystemData(itemType, itemData, unitCost) {
        const cost = { value: Number.isFinite(unitCost) ? unitCost : null, display: String(unitCost ?? "").trim() };
    ...
    _refreshRowSubtotal(html, rowIndex) {
        ...
        const subtotal = row.unitCostValue === null ? "?" : this._formatNuyen(row.unitCostValue * row.quantity);
