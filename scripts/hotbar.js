...
${sr2BuildItemMacroCommand(actor.id, item.id, "attack")}`;
      break;

    case "spell":
      macroName = `${item.name} (Cast)`;
      command = `// Shadowrun 2E Spell Casting Macro
${sr2BuildItemMacroCommand(actor.id, item.id, "cast")}`;
      break;

    case "skill":
      macroName = `${item.name} (Roll)`;
      command = `// Shadowrun 2E Skill Roll Macro
${sr2BuildItemMacroCommand(actor.id, item.id, "base")}`;
      break;

    case "adeptpower":
      macroName = `${item.name} (Activate)`;
      command = `// Shadowrun 2E Adept Power Macro
${sr2BuildItemMacroCommand(actor.id, item.id, "activate")}`;
      break;

    default:
      macroName = `${item.name}`;
      command = `// Shadowrun 2E Item Macro
${sr2BuildItemMacroCommand(actor.id, item.id, "use")}`;
      break;
  }
 ...
 */
window.executeSR2Macro = async function (actorId, itemId, actionType = "default") {
  ...
  const result = await sr2ExecuteActorItemAction(
    actor,
    item,
    actionType === "default" ? sr2GetDefaultActionTypeForItem(item.type) : actionType,
  );
  if (!result.ok && result.reason === "no-handler") {
    ui.notifications.warn(`No runnable action available for ${item.name}.`);
  }
};
