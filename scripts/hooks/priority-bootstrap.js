...
export function sr2BuildCharacterPriorityUpdates(actor, { getAllowedMetatypesForPriority } = {}) {
  ...
  const allowedMetatypes =
    typeof getAllowedMetatypesForPriority === "function"
  ...
  if (sr2IsPriorityLetter(attributePriority)) {
    updates["system.creation.attributePoints"] =
      SR2_PRIORITY_TABLE.attributes[attributePriority] ?? 0;
  }
...
export async function sr2ApplyCharacterPrioritiesOnCreate(
  actor,
  { userId, currentUserId, getAllowedMetatypesForPriority, syncFreeLanguageSkills } = {},
) {
