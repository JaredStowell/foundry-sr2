...
export function registerCreationRuleHooks({
    ...
    const contactLevelsEnabled = typeof areContactLevelsEnabled === "function"
        ? areContactLevelsEnabled
        : () => false;
    const buddiesDisabled = typeof areBuddiesDisabled === "function"
        ? areBuddiesDisabled
        : () => false;
    const contactSummaryForLeader = typeof getContactLevelsSummaryForLeader === "function"
        ? getContactLevelsSummaryForLeader
        : () => null;
    ...
    Hooks.on("preUpdateActor", function (actor, changes, options, userId) {
        ...
        const currentCompleted = sr2ToBool(actor.getFlag?.("shadowrun2e", "creationCompleted")) === true;
