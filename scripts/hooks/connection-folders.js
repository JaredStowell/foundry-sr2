...
export function registerConnectionFolderHooks({ getSystemSetting } = {}) {
  ...
  const readSetting =
    typeof getSystemSetting === "function" ? getSystemSetting : (key, fallback) => fallback;
  ...
  async function sr2EnsureActorConnectionFolderSegment({ name, parentId = null, expectedFlag }) {
    ...
    const candidates = allFolders.filter(
      (f) => f?.type === "Actor" && sr2GetConnectionFolderParentId(f) === parentId,
    );

    const byFlag = candidates.find((f) =>
      sr2ConnectionFolderFlagsMatch(sr2GetConnectionFolderFlag(f), expectedFlag),
    );
